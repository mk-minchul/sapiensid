...
try:
    ...
    from .body_ccvid_vid.evaluate import evaluate
except:
    from base_evaluator import BaseEvaluator
    from body_ccvid_vid.evaluate import evaluate
...
def collate_fn(examples):
    ...
    track_ids = torch.tensor([example["track_id"] for example in examples], dtype=torch.int)
...
class BodyCCVIDVIDEvaluator(BaseEvaluator):
    ...
    def extract(self, pipeline, flip_images=False):
        ...
        for batch_idx, batch in tqdm(enumerate(self.dataloader), total=len(self.dataloader), desc='Extracting Feature',
            ...
            all_track_ids.append(track_ids.cpu().detach())
    ...
    def compute_metric(self, collection, collection_flip):
        ...
        for i, track_id in enumerate(track_ids):
            if track_id.item() not in track_id_to_index:
                track_id_to_index[track_id.item()] = []
            track_id_to_index[track_id.item()].append(i)
...
if __name__ == '__main__':
    evaluator = BodyCCVIDVIDEvaluator(name='body_ccvid_evaluator', data_path='/ssd2/data/body/validation_sets/prcc_test', transform=None, fabric=None, batch_size=128, num_workers=0)
