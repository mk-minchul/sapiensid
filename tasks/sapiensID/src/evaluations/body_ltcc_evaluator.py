...
try:
    ...
    from .body_ltcc.evaluate import evaluate
except:
    from base_evaluator import BaseEvaluator
    from body_ltcc.evaluate import evaluate
...
class BodyLTCCEvaluator(BaseEvaluator):
    def __init__(self, name, data_path, transform, fabric, batch_size, num_workers, save_artifacts_path=''):
        ...
        self.save_artifacts_path = save_artifacts_path
    ...
    def evaluate(self, pipeline, epoch=0, step=0, n_images_seen=0):
        ...
        if self.fabric.local_rank == 0:
            result = self.compute_metric(collection, collection_flip, self.save_artifacts_path)
    ...
    def compute_metric(self, collection, collection_flip, save_artifacts_path):
        if self.is_debug_run():
...
if __name__ == '__main__':
    evaluator = BodyLTCCEvaluator(name='body_ltcc_evaluator', data_path='/ssd2/data/body/validation_sets/prcc_test', transform=None, fabric=None, batch_size=128, num_workers=0)
