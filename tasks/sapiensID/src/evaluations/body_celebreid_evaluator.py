...
try:
    ...
    from .body_celebreid.evaluate import evaluate
except:
    from base_evaluator import BaseEvaluator
    from body_celebreid.evaluate import evaluate
...
def collate_fn(examples):
    ...
    pids = torch.tensor([int(example["pid"].split('/')[-1]) for example in examples], dtype=torch.int)
...
class BodyCelebReIDEvaluator(BaseEvaluator):
...
if __name__ == '__main__':
    evaluator = BodyCelebReIDEvaluator(name='body_celebreid_evaluator', data_path='/ssd2/data/body/validation_sets/market_1501_test', transform=None, fabric=None, batch_size=128, num_workers=0)
