...
class BasePipeline():
    ...
    def resume_from_dir(self, resume_dir):
        ...
        device = getattr(self, self.module_names_list[0]).device
        ...
        for name in self.module_names_list:
            ...
            if hasattr(mod, 'load_state_dict_from_path'):
                ...
                mod_st = torch.load(pipepline_path.replace('pipeline.pt', f'{name}.pt'), map_location=device)
    ...
    def save(self, fabric, pipeline, cfg, epoch, step, n_images_seen, is_best=False, keep_prev_models=False):
        ...
        if fabric.local_rank == 0:
            ...
            # if keep_models is True, copy contents to hist_save_dir excluding specified files
            if keep_prev_models:
                hist_save_dir = os.path.join(cfg.trainers.output_dir, 'checkpoints', f'epoch:{epoch}')
                os.makedirs(hist_save_dir, exist_ok=True)
                for item in os.listdir(save_dir):
                    s = os.path.join(save_dir, item)
                    d = os.path.join(hist_save_dir, item)
                    if not (item.startswith('classifier_') or item == 'optimizer.pt'):
                        if os.path.isdir(s):
    ...
    def save_pipelines_and_configs(save_dir, fabric, pipeline, cfg, epoch, step, n_images_seen):
        ...
        for name in pipeline.module_names_list:
            ...
            if name == 'classifier' and hasattr(pipeline, 'classifiers'):
                # multiple classifiers
                for cls_i, mod in enumerate(pipeline.classifiers):
                    if hasattr(mod, 'save_pretrained'):
                        mod.save_pretrained(save_dir=save_dir, name=f'{name}_{cls_i}.pt', rank=fabric.local_rank)
                continue
