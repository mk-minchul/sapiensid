...
class RecordReader():
    ...
    def read_by_index(self, index):
        ...
        if self.load_aux:
            orig_kps = self.load_keypoints_for_crop(index)
            crop_image, crop_kps = self.center_crop(image, orig_kps)
            # from general_utils.img_utils import visualize, numpy_to_tensor
            # visualize(numpy_to_tensor(crop_image).unsqueeze(0), crop_kps).save('/mckim/temp/temp.png')
            return crop_image, path, crop_kps.squeeze(0)
        return image, path


    def center_crop(self, image, kps):
        h, w, c = image.shape
        ratio = 112 / 160
        h_to_cut = int((h - h * ratio))
        h_pad1 = h_to_cut // 2
        h_pad2 = h_to_cut - h_pad1
        w_to_cut = int((w - w * ratio))
        w_pad1 = w_to_cut // 2
        w_pad2 = w_to_cut - w_pad1
        cropped = image[h_pad1:h-h_pad2, w_pad1:w-w_pad2]

        # New dimensions after cropping
        new_h = h - h_pad1 - h_pad2
        new_w = w - w_pad1 - w_pad2

        # Adjustkps
        adjusted_kps = torch.empty_like(kps)
        adjusted_kps[:, :, 0] = (kps[:, :, 0] * w - w_pad1) / new_w  # Adjust x-coordinates
        adjusted_kps[:, :, 1] = (kps[:, :, 1] * h - h_pad1) / new_h  # Adjust y-coordinates

        return cropped, adjusted_kps

    def load_keypoints_for_crop(self, index):
        orig_kps = torch.from_numpy(self.face_kps_xyn.loc[index, self.keypoint_cols].to_numpy())
        orig_kps = orig_kps.reshape(1, -1, 2)
        return orig_kps.float()
