...
def process_dir(dir_path, relabel=False):
    ...
    for img_path in img_paths:
        pid, _ = filename_to_pid_camid(pattern, img_path)
        pid_container.add(pid)
    pid2label = {pid: label for label, pid in enumerate(pid_container)}

    data = []
    for img_path in img_paths:
        pid, camid = filename_to_pid_camid(pattern, img_path)
        assert 1 <= camid <= 8
...
if __name__ == '__main__':
    ...
    parser.add_argument('--dataset_path', type=str, default='/ssd2/data/body/personReID/occluded_duke')
    parser.add_argument('--dataset_name', type=str, default='occluded_duke_test')
    ...
    train_dataset = process_dir(os.path.join(args.dataset_path, train_dir), relabel=True)
    query_dataset = process_dir(os.path.join(args.dataset_path, query_dir), relabel=False)
    gallery_dataset = process_dir(os.path.join(args.dataset_path, gallery_dir), relabel=False)
