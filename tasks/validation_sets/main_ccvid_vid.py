...
class CCVID(object):
    ...
    def __init__(self, root='/data/datasets/', sampling_step=64, seq_len=16, stride=4, **kwargs):
        ...
        print("  ---------------------------------------------")
        print("  train        | {:5d} | {:11d} | {:9d}".format(num_train_pids, num_train_tracklets, num_train_clothes))
        print("  train_dense  | {:5d} | {:11d} | {:9d}".format(num_train_pids, len(train_dense), num_train_clothes))
        print("  query        | {:5d} | {:11d} | {:9d}".format(num_query_pids, num_query_tracklets, num_query_clothes))
        print("  gallery      | {:5d} | {:11d} | {:9d}".format(num_gallery_pids, num_gallery_tracklets, num_gallery_clothes))
        print("  ---------------------------------------------")
        print("  total        | {:5d} | {:11d} | {:9d}".format(num_total_pids, num_total_tracklets, num_total_clothes))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--dataset_name', type=str, default='ccvid_vid_test')
    parser.add_argument('--save_dir', type=str, default='/ssd2/data/body/validation_sets')
    args = parser.parse_args()

    dataset = CCVID(root=args.dataset_path)

    # Print some statistics
    print('Number of identities: ', dataset.num_gallery_pids)
    print(f"Number of query images: {len(dataset.recombined_query)}")
    print(f"Number of gallery images: {len(dataset.recombined_gallery)}")

    combined = dataset.recombined_query + dataset.recombined_gallery
    full_meta_data =[]
    track_id = -1
    for row in combined:
        tracklet, pid, camid, clothes_id = row
        track_id += 1
        for path in tracklet:
            full_meta_data.append((path, track_id, pid, camid, clothes_id, track_id))

    full_meta_data_label = ['query'] * len(dataset.recombined_query) + ['gallery'] * len(dataset.recombined_gallery)
    meta_data = {
        'full_meta_data': full_meta_data,
        'full_meta_data_label': full_meta_data_label,
        'query': dataset.recombined_query,
        'gallery': dataset.recombined_gallery,
        'num_pids': dataset.num_gallery_pids,
        'num_query': len(dataset.recombined_query),
        'num_gallery': len(dataset.recombined_gallery),
    }


    def entry_for_row(index, row):
        path, track_id, pid, camid, clothes_id, _ = row
